  ...
  if (selectedOption.innerText !== "-- Select an Item index.js --") {
    try {
      ...
      const resp = await fetch(
        `/api/gltf${evt.target.options[event.target.selectedIndex].getAttribute(
          "href"
        )}`
      );
...
fetch(`/api/elements${window.location.search}`, {
  ...
  .then(async (json) => {
    for (const elem of json) {
      if (elem.elementType === "PARTSTUDIO") {
        ...
        child.setAttribute(
          "href",
          `${window.location.search}&gltfElementId=${elem.id}`
        );
        ...
        try {
          const partsResp = await fetch(
            `/api/elements/${elem.id}/parts${window.location.search}`,
            { headers: { Accept: "application/json" } }
          );
          const partsJson = await partsResp.json();
          for (const part of partsJson) {
            const partChild = document.createElement("option");
            partChild.setAttribute(
              "href",
              `${window.location.search}&gltfElementId=${part.elementId}&partId=${part.partId}`
            );
